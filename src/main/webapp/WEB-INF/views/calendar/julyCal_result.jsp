...
<form action="julyCal_update">
<div style="display: flex; flex-flow: column;">
<div style="margin: 0 auto; order:1; row; display: flex;">
<div style="order:1; text-align: center;">
<table width="100%" cellpadding="2" cellspacing="0" border=1" style="background-color: white;">
<tr height="30">
...
<%  for(int i = 1; i <= 6; i++ ) { 
		...
		<font size="2" id="7<%=i%>a">출석</font><br>
		<font size="2" id="7<%=i%>b">결석</font><br>
		<font size="2" id="7<%=i%>c">조퇴 / 지각</font>
		</td> 
	<%}
}%>
</tr>
...
<c:forEach var="a" items="${result }"> <!-- var 은 그냥 이름 items는 lists 란 모델을 가지고오겠다 from service -->
	...
	<script>
	if("${a.attend}" == 'a') {
		document.getElementById("7"+"${a.day}"+"a").style.color="green";	
	}else if ("${a.attend}" == 'b') {
		document.getElementById("7"+"${a.day}"+"b").style.color="red";
	}else if("${a.attend}" == 'c')  {
		document.getElementById("7"+"${a.day}"+"c").style.color="blue";			
	}
...
<div style="order:2; margin-left: 50px; margin-top: 0;">

<h1 style="font-size: 45px;">7월 출석</h1><br>
...
<input type="button" value="출석체크" onclick="location.href='julyCal'"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
