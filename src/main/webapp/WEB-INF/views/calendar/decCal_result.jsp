...
<%  for(int i = 1; i <= 7; i++ ) { 
	...
	if (i % 7 == 0 || i % 7 == 1) { %>
		...
		<font size="2" id="12<%=i%>a">출석</font><br>
		<font size="2" id="12<%=i%>b">결석</font><br>
		<font size="2" id="12<%=i%>c">조퇴 / 지각</font>
		</td> 
<%}
}%>
</tr>
...
<c:forEach var="a" items="${result }"> <!-- var 은 그냥 이름 items는 lists 란 모델을 가지고오겠다 from service -->
	...
	<script>
	if("${a.attend}" == 'a') {
		document.getElementById("12"+"${a.day}"+"a").style.color="green";	
	}else if ("${a.attend}" == 'b') {
		document.getElementById("12"+"${a.day}"+"b").style.color="red";
	}else if("${a.attend}" == 'c')  {
		document.getElementById("12"+"${a.day}"+"c").style.color="blue";			
	}
...
<h1 style="font-size: 45px;">12월 출석</h1><br>
...
<input type="button" value="출석체크" onclick="location.href='decCal'"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
