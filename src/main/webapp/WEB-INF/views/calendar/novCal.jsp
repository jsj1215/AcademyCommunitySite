...
<%  for(int i = 1; i <= 2; i++ ) { 
	String fontcolor = (i % 7 == 2 || i % 7 == 3)? "red" : "black";
		...
		<input type="radio" name="11<%=i%>" id="11<%=i%>a" value="a"><font size="2" id="11<%=i%>aa">출석</font><br>
		<input type="radio" name="11<%=i%>" id="11<%=i%>b" value="b"><font size="2" id="11<%=i%>bb">결석</font><br>
		<input type="radio" name="11<%=i%>" id="11<%=i%>c" value="c"><font size="2" id="11<%=i%>cc">조퇴 / 지각</font>
		</td> 
	<%}
}%>
</tr>
...
<c:forEach var="a" items="${result }"> <!-- var 은 그냥 이름 items는 lists 란 모델을 가지고오겠다 from service -->
	<script>
	if("${a.attend}" == 'a') {
		document.getElementById("11"+"${a.day}"+"a").disabled = true;	
		document.getElementById("11"+"${a.day}"+"b").disabled = true;
		document.getElementById("11"+"${a.day}"+"c").disabled = true;
