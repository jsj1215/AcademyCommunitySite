...
<%  for(int i = 1; i <= 3; i++ ) { 
	...
	<font size="2" id="8<%=i%>a">출석</font><br>
	<font size="2" id="8<%=i%>b">결석</font><br>
	<font size="2" id="8<%=i%>c">조퇴 / 지각</font>
	</td> 
<%}
}%>
</tr>
...
<c:forEach var="a" items="${result }"> <!-- var 은 그냥 이름 items는 lists 란 모델을 가지고오겠다 from service -->
	...
	<script>
	if("${a.attend}" == 'a') {
		document.getElementById("8"+"${a.day}"+"a").style.color="green";	
	}else if ("${a.attend}" == 'b') {
		document.getElementById("8"+"${a.day}"+"b").style.color="red";
	}else if("${a.attend}" == 'c')  {
		document.getElementById("8"+"${a.day}"+"c").style.color="blue";			
	}
...
<h1 style="font-size: 45px;">8월 출석</h1><br>
...
지각/조퇴 차감액 : <c:out value="${moneyc }"/> 원<br>
<br>

<input type="button" value="출석체크" onclick="location.href='augCal'"> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
